#!/bin/sh
# Algorithme d'automatisation (déchiffrement) de CAST5 mode CBC via une database kdbx à trois facteurs d'authentification
...
until [[ $N -lt $i ]]
	do
		...
		openssl enc -bf-cbc -d -in $FILEPATH -out ${FILEPATH}.enc -salt -pbkdf2 -k $X
