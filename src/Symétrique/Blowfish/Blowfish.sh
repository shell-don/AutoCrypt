#!/bin/sh
# Algorithme d'automatisation (chiffrement) de Blowfish mode CBC via une database kdbx à trois facteurs d'authentification
...
until [[ $i -gt $N ]] 
	do
		...
		openssl enc -bf-cbc -e -in $FILEPATH -out ${FILEPATH}.enc -salt -pbkdf2 -k $X
